...
import vistrack
from vistrack import Detection, Tracker, Video
from vistrack.drawing import Color
...
def yolo_detections_to_vistrack_detections(
    yolo_detections: torch.tensor,
) -> List[Detection]:
    """convert detections_as_xywh to vistrack detections"""
    vistrack_detections: List[Detection] = []
    ...
    for detection_as_xyxy in detections_as_xyxy:
        ...
        vistrack_detections.append(Detection(points=bbox, scores=scores, label=label))

    return vistrack_detections
...
if __name__ == "__main__":
    ...
    for input_path in args.files:
        ...
        for frame in video:
            ...
            yolo_detections = yolo_detections_to_vistrack_detections(yolo_out)
            detections = openpose_detections + yolo_detections

            tracked_objects = tracker.update(detections=detections)

            vistrack.draw_tracked_objects(
                frame,
                [person for person in tracked_objects if person.label == -1],
                color=Color.green,
            )
            vistrack.draw_tracked_boxes(
                frame,
