...
class FilterPyKalmanFilterFactory(FilterFactory):
    ...
    def create_filter(self, initial_detection: np.ndarray) -> KalmanFilter:
        """
        This method returns a new predictive filter instance with the current setup, to be used by each new [`TrackedObject`][vistrack.tracker.TrackedObject] that is created.
        ...
        initial_detection : np.ndarray
            numpy array of shape `(number of points per object, 2)`, corresponding to the [`Detection.points`][vistrack.tracker.Detection] of the tracked object being born,
...
class OptimizedKalmanFilterFactory(FilterFactory):
    """
    Creates faster Filters than [`FilterPyKalmanFilterFactory`][vistrack.filter.FilterPyKalmanFilterFactory].
