...
from vistrack.drawing.color import Palette
from vistrack.drawing.drawer import Drawer
from vistrack.tracker import TrackedObject
from vistrack.utils import warn_once
...
class Paths:
    ...
    get_points_to_draw : Optional[Callable[[np.array], np.array]], optional
        Function that takes a list of points (the `.estimate` attribute of a [`TrackedObject`][vistrack.tracker.TrackedObject])
    ...
    color : Optional[Tuple[int, int, int]], optional
        [Color][vistrack.drawing.Color] of the circles representing the paths of interest.
    ...
    Examples
    --------
    >>> from vistrack import Tracker, Video, Path
    ...
    def draw(
        ...
        tracked_objects : Sequence[TrackedObject]
            List of [`TrackedObject`][vistrack.tracker.TrackedObject] to get the points of interest in order to update the paths.
...
class AbsolutePaths:
    ...
    Works just like [`Paths`][vistrack.drawing.Paths] but supports camera motion.
